...
function iniciarApp() {
    ...
    // Muestra el resumen de servicios o mensaje de error(en caso de no poder validar)
    mostrarResumen();
...
function cambiarSeccion() {
    ...
    enlaces.forEach( enlace => {
        enlace.addEventListener('click', e => { // Itera en cada boton y escucha por evento click
            e.preventDefault(); // Previene el default del boton submit

            pagina = parseInt(e.target.dataset.paso); // convierte de string a integral

            // Llamar la funcion de mostrar seccion

            mostrarSeccion();

            // Cambiar botones paginador
...
async function mostrarServicios() {
    try {
        ...
        const db = await resultado.json(); // Al usar fetch debo especificar el tipo de resultado
        const  { servicios } = db;
...
function seleccionarServicio(e) {
    ...
    let elemento;
    // Forzar que el elemento al cual le damos click corresponda al DIV
...
function nombreCita() {
    ...
    nombreInput.addEventListener('input', e => {
        const nombreTexto = e.target.value.trim(); // trim,trimStart,trimEnd : cortan los espacios en blanco

        // Validacion de contenido de nombreTexto
